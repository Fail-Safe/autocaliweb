    ...
    def run(self, worker_thread):
        ...
        if config.config_use_google_drive:
            with app.app_context():
                ...
                df_cover = gdriveutils.getFileFromEbooksFolder(
            ...
            if os.path.isfile(file_path + format_new_ext) or local_db.get_book_format(
                ...
                if not new_format:
                    ...
                    try:
                        ...
                        # If the Calibre DB is mounted read-only, conversion output can still exist on disk
                        # but writing the new format row to `calibre.data` will fail. Treat that as a
                        # successful conversion (the caller can serve the on-disk file) rather than
                        # turning it into a fatal task error.
                        if (
                            ...
                            log.warning(
                                ...
                                book_id,
                                self.settings["new_book_format"].upper(),
                                e,
                            )
                        else:
                            local_db.session.rollback()
                            ...
                            self._handleError(
                                N_("Oops! Database Error: %(error)s.", error=e)
                            )
                            return
                    self._handleSuccess()
                    local_db.session.close()
            ...
            if check == 0:
                ...
                if os.path.isfile(file_path + format_new_ext):
                    new_format = (
                        ...
                        .filter(db.Data.book == book_id)
                        .filter(
                            db.Data.format == self.settings["new_book_format"].upper()
                        )
                        .one_or_none()
                    )
                    if not new_format:
                        new_format = db.Data(
                            ...
                            book_format=self.settings["new_book_format"].upper(),
                            book=book_id,
                            uncompressed_size=os.path.getsize(
                    ...
                    error_message = N_(
                        "%(format)s format not found on disk",
    ...
    def _convert_kepubify(self, file_path, format_old_ext, format_new_ext):
        ...
        if check == 0:
            converted_file = glob.glob(
                glob.escape(os.path.splitext(filename)[0]) + "*.kepub.epub"
            )
                ...
                if check == 0:
                    ...
                    for ele in calibre_traceback:
                        ...
                        if not ele.startswith("Traceback") and not ele.startswith(
                            ...
                            error_message = N_(
                                "Calibre failed with error: %(error)s", error=ele
                            )
            ...
            if config.config_embed_metadata:
                ...
                if has_cover:
                    ...
                    command.extend(
                        [
                            "--cover",
                            os.path.join(os.path.dirname(file_path), "cover.jpg"),
                        ]
                    )
                    quotes_index = 7
