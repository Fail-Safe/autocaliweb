...
def url_for_other_page(page):
    """
    Build a URL for the current endpoint, switching only the page number.

    This needs to be resilient when the current request.view_args contain values
    that don't match the converter types for the current endpoint (e.g. a
    generated shelf identifier accidentally ending up in an <int:shelf_id>
    param). In that case, fall back to rebuilding the URL without path params
    and preserve query args.
    """
    args = request.view_args.copy() if request.view_args else {}
    args["page"] = page

    # Preserve query parameters (except "page", which we override)
    ...
    try:
        ...
        # If path parameters are incompatible with the current route converters,
        # retry without view_args to avoid blowing up template rendering.
        safe_args = {"page": page}
        for get, val in request.args.items():
            if get == "page":
                continue
            safe_args[get] = val
        return url_for(request.endpoint, **safe_args)
