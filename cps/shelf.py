...
from .generated_shelves import GeneratedShelf, generated_shelf_filter, list_generated_shelves
...
def list_shelves():
    ...
    try:
        ...
        owner_names = {}
        if owner_ids:
            owners = ub.session.query(ub.User.id, ub.User.name).filter(ub.User.id.in_(owner_ids)).all()
        ...
        for s in regular_shelves:
            ...
            rows.append({
                ...
                "can_edit": is_own or (s.is_public and current_user.role_edit_shelfs()),
                "source": "",
                "value": "",
            })
            ...
            try:
                for gs in generated_shelves:
                    ...
                    if shelf_filter is not None:
                        count = calibre_db.session.query(db.Books).filter(
                            calibre_db.common_filters(), shelf_filter
                        ).count()
        ...
        log.debug("list_shelves: returning %d rows (total: %d)", len(paginated_rows), total_count)
        return jsonify({
            "total": total_count,
...
def bulk_kobo_sync():
    ...
    # -------------------------------------------------------------------------
    if regular_ids:
        editable_shelves = ub.session.query(ub.Shelf).filter(
    ...
    return jsonify({
        "success": True,
        "modified": modified,
        "msg": _("Kobo sync %(action)s for %(count)s shelves", action=action, count=modified),
    })
...
def bulk_delete_shelves():
    ...
    deletable = ub.session.query(ub.Shelf).filter(
...
def delete_shelf_helper(cur_shelf):
    ...
    if cur_shelf.name == KOBO_OPT_IN_SHELF_NAME:
        mode = get_user_kobo_collections_mode(current_user)
        if mode == "hybrid":
            flash(_("The '%(name)s' shelf cannot be deleted while in 'Sync Selected Books' mode.", name=KOBO_OPT_IN_SHELF_NAME), category="error")
            return False
