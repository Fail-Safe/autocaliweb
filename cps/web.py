...
def add_security_headers(resp):
    ...
    if request.endpoint == "edit-book.show_edit_book" or request.path.startswith(
        "/metadata/"
    ):
    ...
    if (
        request.endpoint == "edit-book.show_edit_book"
        or request.endpoint == "web.service_worker"
...
def acw_get_library_location() -> str:
    dirs = {}
    DIRS_JSON = os.path.join(
        os.environ.get("ACW_INSTALL_DIR", "/app/autocaliweb"), "dirs.json"
    )
...
def render_books_list(data, sort_param, book_id, page):
    ...
    if data == "rated":
        ...
        # Redirect out-of-range pages to the last available page for a better UX.
        # (e.g. /page/999 -> /page/<last>)
        try:
            last_page = int(getattr(pagination, "pages", 0) or 0)
        except Exception:
            last_page = 0
        ...
        if last_page > 0 and page > last_page:
            ...
            if request.endpoint == "web.index":
                return redirect(url_for("web.index", page=last_page, **args))
            return redirect(
                url_for(
                    "web.books_list",
                    data=website,
...
def render_author_books(page, author_id, order):
    ...
    if services.hardcover and config.config_use_hardcover:
        hardcover = services.hardcover.HardcoverClient(
            config.config_hardcover_api_token
        )
...
def books_table():
    ...
    return render_title_template(
        ...
        kobo_sync_hybrid=kobo_sync_hybrid,
    )
...
def list_book_ids():
    ...
    if search_param:
        ...
        q = calibre_db.session.query(db.Books.id).filter(
            calibre_db.common_filters(allow_show_archived=True)
        )
    ...
    try:
        ...
        return jsonify(
            {"success": False, "msg": _("Oops! Database Error: %(error)s.", error=ex)}
        ), 500
        ...
        if resolution in ("sm", "md", "lg") and response is not None:
            ...
            if cache_busted:
                response.headers["Cache-Control"] = (
                    "private, max-age=2592000, immutable"  # 30 days
                )
            else:
...
def send_to_selected_ereaders(book_id):
    ...
    if result is None:
        ...
        response = [
            {
                "type": "success",
                "message": _("Success! Book queued for sending to selected addresses"),
            }
        ]
    else:
        response = [
...
def login():
    ...
    if (
        config.config_oauth_auto_redirect and 3 in oauth_check
    ) and not redirect_parameter:
...
def change_profile(
    ...
    try:
        ...
        current_user.kobo_generated_shelves_sync = (
            int(to_save.get("kobo_generated_shelves_sync") == "on") or 0
        )
        current_user.kobo_generated_shelves_all_books = (
            int(to_save.get("kobo_generated_shelves_all_books") == "on") or 0
        )
        current_user.kobo_sync_empty_collections = (
            int(to_save.get("kobo_sync_empty_collections") == "on") or 0
        )
        try:
            current_user.kobo_sync_item_limit = int(
        ...
        current_user.hardcover_token = (
            to_save.get("hardcover_token", "").replace("Bearer ", "") or ""
        )
